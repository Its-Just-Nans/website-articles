...
On our browser, we simply need to type `n4n5.lastname/` (the slash is mandatory to force the user to accept this url, since this extension does not exist) and we should see our website.

But that's not the case! First of all, we see that the website is not secure, this is because the certificate is not valid for this domain. We can ignore this warning (see last paragraph) and continue to the website.
...
We got 404 we got an error! But now we know why! We need to add the `Host` header to our request!
